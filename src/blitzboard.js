...
class Blitzboard {
  ...
  constructor(container) {
    ...
    this.clientIsMacLike = /(Mac|iPhone|iPod|iPad)/i.test(
    ...
    this.network = new DeckGL.Deck({
      ...
      getTooltip: (elem) => {
        if (
          blitzboard.contextMenu.isOpened ||
          !elem?.object ||
          (!blitzboard.config.edge.canFocus &&
            elem.object.objectType === "edge")
        )
          return null;
        return {
  ...
  update(applyDiff = true, afterUpdate = null) {
    ...
    this.filteredGraph = {};
    this.filteredGraph.nodes = this.graph.nodes.filter(
      (node) => !blitzboard.isFilteredOutNode(node)
    );

    this.filteredGraph.edges = this.graph.edges.filter(
      (edge) =>
        !blitzboard.isFilteredOutEdge(edge) &&
        !blitzboard.isFilteredOutNode(this.nodeMap[edge.from]) &&
        !blitzboard.isFilteredOutNode(this.nodeMap[edge.to])
    );
